...
DELAY_BETWEEN_ACCOUNTS = 2           # Seconds between sending to different accounts
...
async def send_message_to_account(client: Client, phone: str, message: str) -> Dict[str, Any]:
...
async def send_with_retry(client: Client, phone: str, message: str) -> Dict[str, Any]:
    """Send a message with retry logic."""
    for attempt in range(MAX_RETRIES_PER_ACCOUNT + 1):
        result = await send_message_to_account(client, phone, message)
        if result["success"]:
            return result
...
async def main():
    ...
    async with Client(SESSION_NAME) as client:
        for i, phone in enumerate(phone_numbers, 1):
            ...
            result = await send_with_retry(client, phone, message)
